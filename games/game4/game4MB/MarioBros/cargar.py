...
class spriteSheet:
    ...
    def tsv(self, c, f): # correccion de la trasposicion fila-columna
        return self.matrix_tsv[f][c]
    
    def tsv_float(self, c, f): # generalización del casteo a float
        return float(self.tsv(c, f).replace(",", ".")) 
    
    def extraer_usando_tsv(self, nombre, w=-1, h=-1): #extrae una subimagen de la imagen principal
    ...
    def propiedades(self, nombre): # extraccion de los datos de una imagen dentro de la hoja
        fila = 0
        datos = comodin()
        try:
            while true:
...
def cargaImagenes():
    ...
    gestorImagenes.principal = spriteSheet(
                                           "imagenes", #nombre
                                           #url imagen o directamente imagen
                                           ...
                                           -1, #redimensionamiento ancho
                                           -1, #redimensionamiento alto
                                           #url metricas tsv
...
def cargarNivel1():
    ...
    #------------MAPEO DE LOS BLOQUES CABECEABLES------------
    ...
    cajas.append(BloqueCabeceable(64, 8, gestorImagenes.cajaMisteriosa).invisible(true)) #Vida oculta  
    for i in range(3):
    ...
    consultarBloqueCabeceable(94, 9).contadorDeUso(10) #alcancia
    for i in range(2):
    ...
    #------------MAPEO DE LOS PATRONES DE FONDO------------
