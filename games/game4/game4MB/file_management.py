...
def matrix_tsv(url, local=null):
    if(local == null):
        #si no se indica una ubicacion se intuye que el archivo no se va a descargar
        lines = lines_utf_8_web(url)
    else:
        if(not location_exists(local)):
            # si la ubicacion no existe, se procede a descargar el archivo de internet
            download(url, local)
        #se lee el archivo en la ubicacion
    ...
    for line in lines:
        celdas.append(
                      ...
                      .replace("\r", "") # se elimina el caracter \r
                      .split("\t")  # se separan en celdas diferentes los elementos separados por tabulaciones
                      )
