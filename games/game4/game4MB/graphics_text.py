...
def heightText(): #funcion de sinonimia
    return font_height()
...
def draw_text_in_rect_center(string, w, h, AH=izquierda, AV=arriba):  #dibuja una imagen de texto centrada que se ajusta al anchor de un area rectangular, saltando automaticamente de renglon y ademas admite los saltos de renglon a voluntad
    x = (width()-w) / 2
    y = (height()-h) / 2
    paintOtherCanvas(text_in_rect(string, w, h, AH, AV), x, y)
    
def draw_text_in_rect(string, x, y, w, h, AH=izquierda, AV=arriba): #dibuja una imagen de texto en la posicion x,y que se ajusta al anchor de un area rectangular, saltando automaticamente de renglon y ademas admite los saltos de renglon a voluntad
    paintOtherCanvas(text_in_rect(string, w, h, AH, AV), x, y)
    
def text_in_rect(string, w, h, AH=izquierda, AV=arriba): #genera una imagen de texto que se ajusta al anchor, saltando automaticamente de renglon y ademas admite los saltos de renglon a voluntad
...
def drawTextCenter(string, AH=izquierda): #dibuja centrada una imagen de texto que admite saltos de linea
...
def drawText(string, x, y, AH=izquierda, transformar=true): #dibuja en la posicion x,y una imagen de texto que admite saltos de linea
    try:
        if(not transformar):
...
def textRender(string, AH=izquierda): #genera una imagen con texto que admite saltos de linea
    w = 0
    h = 0
    lines = string.splitlines()
    for line in lines:
