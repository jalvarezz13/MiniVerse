...
def main():
    ...
    pathname = os.path.join(root, "tituloTetris.jpg")
    title_image = pygame.image.load(pathname)
    pygame.display.set_caption("Tetris - MiniVerse")
    ...
    try:
        ...
        fontTittle = pygame.font.SysFont('Sans Serif', 30)
        fontFinal = pygame.font.SysFont('Consolas', 18)
    except OSError:
        # If the font file is not available, the default will be used.
        pass

    tetris_block_text = fontTittle.render("TETRIS", True, (0,255,255), bgcolor)
    miniverse_block_text = fontTittle.render("MiniVerse", True, (46, 204, 113), bgcolor)
    next_block_text = font.render("Siguiente figura:", True, (255, 255, 255), bgcolor)
    score_msg_text = font.render("PUNTOS:", True, (255, 255, 255), bgcolor)
    game_over_text = fontFinal.render("¡FIN DE LA PARTIDA!", True, (255, 220, 0), bgcolor)
    ...
    while run:
        ...
        draw_centered_surface(screen, tetris_block_text, 50)
        draw_centered_surface(screen, miniverse_block_text, 100)
        draw_centered_surface(screen, next_block_text, 200)
        draw_centered_surface(screen, blocks.next_block.image, 250)
        draw_centered_surface(screen, score_msg_text, 390)
        score_text = font.render(
            str(blocks.score), True, (255, 255, 255), bgcolor)
        draw_centered_surface(screen, score_text, 420)
        if game_over:
            draw_centered_surface(screen, game_over_text, 510)
        # Update.
        pygame.display.flip()
